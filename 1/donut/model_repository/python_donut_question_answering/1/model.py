...
class TritonPythonModel:
    def initialize(self, args):
        self.feature_extractor = DonutProcessor.from_pretrained("naver-clova-ix/donut-base-finetuned-docvqa")
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            question = str(question.as_numpy())
            input_image = np.squeeze(inp.as_numpy()).transpose((2, 0, 1))

            task_prompt = "<s_docvqa><s_question>{user_input}</s_question><s_answer>"
            prompt = task_prompt.replace("{user_input}", question)

            decoder_input_ids = self.feature_extractor.tokenizer(prompt, add_special_tokens=False,
